...
def chat():
    try:
        ...
        # 2. CHẾ ĐỘ LLM: Nếu có chữ "ollama" HOẶC phiên đang ở chế độ LLM
        if "ollama" in message or session_id in ACTIVE_LLM_SESSIONS:
            ACTIVE_LLM_SESSIONS.add(session_id)
            return jsonify({"use_llm": True, "query": message})

        # 3. CHẾ ĐỘ TÌM SÁCH BÌNH THƯỜNG
        num_books = 1 
        num_match = re.search(r"(\d+)\s*(quyển|cuốn|sách)?", message)
        if num_match:
        ...
        if category_query:
            ...
            if matched_prefix:
                ...
                if results:
                    ...
                    limited_results = results[:num_books]
                    return jsonify({
                        "books": limited_results,
                        "message": f"📚 {len(limited_results)} sách {display_name}:"
                    })
                else:
        ...
        if results:
            ...
            for b in results:
                if b["class_code"] not in seen:
                    ...
                    unique.append(b)
            return jsonify({"books": unique[:num_books]})

        # 4. Không tìm thấy sách, rơi vào LLM -> Chuyển phiên này sang LLM
        ACTIVE_LLM_SESSIONS.add(session_id)
        return jsonify({"use_llm": True, "query": message})
...
"""
    ...
    chat_help = """
💬 **HƯỚNG DẪN TRÒ CHUYỆN VỚI AI AGENT**
...
"""
    ...
    library_help = """
**HƯỚNG DẪN SỬ DỤNG THƯ VIỆN THỰC TẾ**

**Địa chỉ:** Thư viện Trường Đại học Hải Phòng (Hiện ở tòa nhà A8 của Trường Đại học Hải Phòng).
...
⚠️ **Lưu ý:** 
- Tôi chỉ trả lời dựa trên dữ liệu sách có sẵn. 
- Nếu xảy ra lỗi, vấn đề về thư viện mà tôi trả lời không chính xác, hãy trao đổi trực tiếp với các cán bộ thư viện trực quầy để được hỗ trợ một cách đúng và đầy đủ nhất. 
 **Xin cám ơn sự ủng hộ của bạn!**
"""
